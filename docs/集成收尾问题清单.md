...
4. **QuerySystem 已接入但未形成闭环（只生成 QuerySchema）**
   - 现状：主链路会为 user ChatMessage 构建 `query_schema`（signal_density/intent/retrieve 等）。
   - 风险：QuerySchema 不驱动检索/工具调用时，会变成“只有日志，没有收益”的额外复杂度。
   - 建议：明确 QuerySchema 的消费点（例如 MemorySystem/ContextAssembler/ToolRouter），把 `retrieve/mode/top_k/token_budget` 等字段打通。

5. **QuerySchema 未落库，调试回放困难**
   - 现状：ChatMessage 有 `query_schema` 字段，但 `to_json()/from_dict()` 未包含该字段。
   - 建议：确定持久化方案（写入 extra/json 列、单独表，或仅保存 debug_tags），以便回放与评估。
...
4. **Prompt 配置化后的“单一来源”与模板渲染安全**
   - 现状：已引入 `config/system_prompt.yaml` 作为外置 prompt 的入口，但仍可能与代码 builder 并存。
   - 风险：双源漂移（yaml 与代码不一致）+ 模板中花括号与字段替换的安全问题。
   - 建议：明确 source of truth；渲染时仅替换 required_fields，避免对模板执行不受控的 `str.format()`。

5. **LLMManagement 模型实现映射策略未完全收敛**
   - 风险：模型名/实现类映射若不稳定，后续扩模型/切模型容易反复改代码。
   - 建议：选定“配置映射”或“supportModel()/自动发现”其中一种并固化；同时给出可观测的启动日志与错误提示。

6. **PostTreatmentSystem 的可靠性与闭环**
   - 现状：回合后已能异步调度 PostHandleSystem，但 POST_HANDLE_COMPLETED 的消费逻辑仍偏骨架。
   - 建议：补齐 handler 超时、失败隔离、幂等与重试策略，并明确产物落点（例如写库/写文件/发 websocket）。
...
7. 打通 QuerySchema 的消费闭环（至少在 ContextAssembler/MemorySystem 层有明确收益）
8. 收敛 PostTreatmentSystem 的产物落点与可靠性（失败隔离/重试/完成事件消费）
