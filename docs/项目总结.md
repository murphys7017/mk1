...
1. 终端输入文本（[main.py](../main.py)）
  - 从环境变量读取 `OPENAI_API_KEY`（支持 `.env`）
2. 调用 `Alice.respond(user_inputs)`（异步）
3. `PerceptionSystem.analyze()` 并发执行分析器（目前主要是 text）→ 得到 `ChatMessage` 列表
4. 选择聚合输入（当前默认取最后一个）
5. （建设中）构建全局上下文 messages
  - `ContextAssembler/DefaultGlobalContextAssembler` 已能把“记忆块（MemorySystem）+ 对话状态块（ChatStateSystem）”合并为 system prompt，并拼接最近原始对话
...
> 现状提醒：已修复一个重要一致性问题：`summarize_dialogue` 的调用方（`DialogueStorage.summarize_dialogue`）已改为使用 `summary_text` 和 `dialogues_text` 参数名，与 `SystemPrompt` 的 `PromptTemplate.required_fields` 对齐。
...
- [ChatStateSystem/ChatStateSystem.py](../ChatStateSystem/ChatStateSystem.py)
  - 抽象接口：定义 `getState()/checkAndUpdateState()/assemble()`
- [ChatStateSystem/DefaultChatStateSystem.py](../ChatStateSystem/DefaultChatStateSystem.py)
  - 默认实现：基于最近 N 轮对话调用 `LLMManagement` 的 `judge_chat_state` prompt
  - `assemble()` 输出结构化的 `<CHAT_STATE>` prompt 片段（由 `ChatState.to_prompt()` 生成）
...
- [ContextAssembler/DefaultGlobalContextAssembler.py](../ContextAssembler/DefaultGlobalContextAssembler.py)
  - 将 `MemorySystem.assemble()`（记忆块）+ `ChatStateSystem.assemble()`（状态块）合并为 system prompt，再拼接最近原始对话 history 组成 messages
