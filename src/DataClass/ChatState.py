...
class ChatState:
	...
	def to_prompt(self) -> PromptBuilder:
		b = PromptBuilder(TagType.CHAT_STATE_TAG)
		b.add(f"当前对话状态：")
		b.add(f"	- 互动类型：{self.interaction}")
		b.add(f"	- 用户态度：{self.user_attitude}")
		b.add(f"	- 情绪状态：{self.emotional_state}")
		b.add(f"	- 主导方式：{self.leading_approach}")
		return b
